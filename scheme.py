...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        evalt_operands = operands.map((lambda op: scheme_eval(op, env)))
        return scheme_apply(self, evalt_operands, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_pair = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(lambda_pair, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        formals = target.second
        macro_proc = MacroProcedure(formals, expressions.second, env)
        env.define(target.first, macro_proc)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
